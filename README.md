...
* [`src/web_client/`](src/web_client/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/web_client/core.cljs): contains the SPA entry point, `init`
* [`test/web_client/`](test/web_client/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `web-client` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/web_client/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns web-client.example
  (:require [web-client.config :as config])
