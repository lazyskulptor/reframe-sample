(ns web-client.view.panels.signup
  (:require
   [web-client.base.layout :as l]))

(defmethod l/panels :signup-panel []
  [l/centered
   (fn []
   [:div
    {:class "h-32 md:h-auto md:w-1/2"} [:img {:aria-hidden "true", :class "object-cover w-full h-full dark:hidden", :src "/img/create-account-office.jpeg", :alt "Office"}] [:img {:aria-hidden "true", :class "hidden object-cover w-full h-full dark:block", :src "/img/create-account-office-dark.jpeg", :alt "Office"}]])
   (fn [] [:div
    ...
    [:div
     ...
     [:button
      {:class
       "flex items-center justify-center w-full px-4 py-2 text-sm font-medium leading-5 text-black text-gray-700 transition-colors duration-150 border border-gray-300 rounded-lg dark:text-gray-400 active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray"}
      [:svg
       {:class "w-4 h-4 mr-2",
        :aria-hidden "true",
        :viewBox "0 0 24 24",
     ...
     [:button
      {:class
       "flex items-center justify-center w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-black text-gray-700 transition-colors duration-150 border border-gray-300 rounded-lg dark:text-gray-400 active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray"}
      [:svg
       {:class "w-4 h-4 mr-2",
        :aria-hidden "true",
        :viewBox "0 0 24 24",
